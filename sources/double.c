...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/25 19:10:17 by dwisoky           #+#    #+#             */
/*   Updated: 2019/10/25 19:11:02 by dwisoky          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_number *gg)
{
	...
	if (precision == 1)
	{
		fractional->num += look_for_next(fractional->next, fractional);
		if ((tmp = fractional->num / 10))
			fractional->num %= 10;
		free_num(&fractional->next);
		return (tmp);
	}
	tmp = frac_rounding(fractional->next, --precision, gg);
	if (tmp)
		fractional->num += tmp;
		...
		t_number **fractional, unsigned int precision)
{
	...
	if (!precision)
	{
		...
		tmp = frac_rounding(*fractional, precision, integral);
